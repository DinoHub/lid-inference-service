...
LID Inference Service for AI App Store
...
```sh
docker tag lid-inference-service:1.0.0 <REGISTRY>/<REPO>/lid-inference-service:1.0.0
```
...
```sh
docker push <REGISTRY>/<REPO>/lid-inference-service:1.0.0
```
...
2. Pass the docker image URI (e.g `<REGISTRY>/<REPO>/lid-inference-service:1.0.0`) when creating/editing the inference service
